...
import { acceptsLanguages } from "https://deno.land/std@0.213.0/http/mod.ts";
...
Deno.serve({}, async function(req: Request) {
    ...
    if (path.startsWith(PATH_STATIC)) {
        ...
        if (response === null)
            return pageResponse(req, NOT_FOUND_PAGE);
    ...
    return pageResponse(req, path || 'index');
});
